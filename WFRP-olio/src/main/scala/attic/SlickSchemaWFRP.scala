...
/**
 * DEPRECATED - NO LONGER USING SLICK
 */
object SlickSchemaWFRP {
  ...
  class TableCareer(tag: Tag) extends Table[CareerRow](tag, "CAREER") {
    ...
    def careerExits = column[Option[String]]("CareerExits", O.Length(64, true))
    
    def * = (careerId, careerName, careerDescription, careerSkills, careerTalents, careerExits) <>
        ( { t => CareerRow( t._1, t._2, t._3,
                            // skills (non-options)
                            GetCommaOpsInt(t._4).map( SkillRow.Id(_) ),
                            // skills (optional, as in "Skill A or Skill B")
                            GetCommaOpsArrayedInt(t._4).map( _.map(SkillRow.Id(_)) ),
                            // talents (non-options)
                            GetCommaOpsInt(t._5).map( TalentRow.Id(_) ),
                            // talents (optional, same as skills)
                            GetCommaOpsArrayedInt(t._5).map( _.map(TalentRow.Id(_)) ),
                            // career exits
                            CommaTextOptToIntArray(t._6).map( CareerRow.Id(_) )
                          )
          },
          { aRow: CareerRow => Some(( aRow.id, aRow.name, aRow.description, //IntArrayToCommaTextOpt( aRow.skills.map(_.value) ),
                                      DataAndOptionalIntsToCommaOps( aRow.skills.map(_.value), aRow.skillOptions.map( _.map(_.value) ) ),
                                      DataAndOptionalIntsToCommaOps( aRow.talents.map(_.value), aRow.talentOptions.map( _.map(_.value) ) ),
                                      IntArrayToCommaTextOpt( aRow.careerExits.map(_.value) )
                                    ))
          }
        )
  }
  ...
  class TableOlio(tag: Tag) extends Table[OlioRow](tag, "OLIO") {
    ...
    def olioCareers = column[Option[String]]("OlioCareers", O.Length(32, true))
    ...
    def * = (olioId, olioName, olioCareers, olioTalents) <> //.mapTo[OlioRow]
            ( { t => OlioRow( t._1, t._2, CommaTextOptToIntArray(t._3).map( CareerRow.Id(_) ),
                              CommaTextOptToIntArray(t._4).map( TalentRow.Id ) ) },
              { aRow: OlioRow => Some(( aRow.id, aRow.name, IntArrayToCommaTextOpt(aRow.careers.map( _.value )),
                                        IntArrayToCommaTextOpt( aRow.talents.map( _.value ) ) )) } )
  }
